...
DISTRACT_TIME = 60    # 干扰任务（演示用1分钟，正式用3分钟）
...
class ReyFigureWidget(QWidget):
    """用 QPainter 渲染 Rey-Osterrieth 复杂图形"""
...
class DrawingCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.strokes = []       # [{'path': QPainterPath, 'pen': QPen, 'eraser': bool}]
        ...
        self.tool = 'pen_thin'  # pen_thin, pen_thick, eraser

        self.drawing = False

    def set_tool(self, tool):
        """切换工具: pen_thin / pen_thick / eraser"""
        self.tool = tool
        if tool == 'eraser':
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if self.tool == 'pen_thick':
                w = self.pen_thick
                self.current_pen = QPen(Qt.black, w, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
            elif self.tool == 'pen_thin':
                w = self.pen_thin
                self.current_pen = QPen(Qt.black, w, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
            else:
                w = self.eraser_width
                self.current_pen = QPen(Qt.white, w, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
...
class ToolBar(QFrame):
    tool_changed = Signal(str)  # pen_thin / pen_thick / eraser
    undo_clicked = Signal()
    redo_clicked = Signal()
    ...
    def _select(self, tool):
        ...
        # 强制刷新样式
        self.btn_thin.style().unpolish(self.btn_thin)
        self.btn_thick.style().unpolish(self.btn_thick)
        self.btn_eraser.style().unpolish(self.btn_eraser)
        self.btn_thin.style().polish(self.btn_thin)
        self.btn_thick.style().polish(self.btn_thick)
        self.btn_eraser.style().polish(self.btn_eraser)
...
class ExperimentWidget(QWidget):
    """三分区实验界面：刺激图 + 画布 + 工具栏"""
    phase_done = Signal(dict)   # 阶段完成，携带 canvas.snapshot()
    back_menu = Signal()

    def __init__(self, parent=None):
        super().__init__(parent)
        self.phase = ''         # 'copy' or 'recall'
        self.time_limit = 0
        self.elapsed = 0
        self.subject = {}
        self.timestamp = ''
        ...
        self.clock.setInterval(200)  # 200ms 刷新
...
class MenuWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        title = QLabel('Rey-Osterrieth 复杂图形测验 (ROCF)')
        title.setObjectName('titleLabel')
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)
...
class ROCFMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('ROCF 电子化测评系统')
    ...
    def _save_screenshot(self, phase):
        ...
        path = os.path.join(OUTPUT_DIR,
                           f'rocf_{self._subject["id"]}_{phase}_{self._timestamp}.png')
    ...
    def _save_and_report(self):
        """保存 JSON 数据并弹窗"""
        path = os.path.join(OUTPUT_DIR,
                           f'rocf_{self._subject["id"]}_{self._timestamp}.json')
        data = {
